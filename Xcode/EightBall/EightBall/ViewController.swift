...
class ViewController: UIViewController {
    ...
    @IBAction func checkButtonAction(_ sender: Any) {
        let max: Int = possibleAnswers.count - 1
        let answer = possibleAnswers[Int.random(in: 0...max)]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "AnswerSegue" {
            if let destination = segue.destination as? AnswerTVC {
                destination.groupedAnswers = Dictionary(grouping: possibleAnswers, by: { $0.type })
                destination.allAnswerTypes = Array(destination.groupedAnswers.keys)
            }
        }
    }
